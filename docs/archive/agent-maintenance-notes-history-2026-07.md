# Archived Agent Maintenance Notes — through 2026-07-29
...
## 2026-07-29 - Cascade provenance and live-shaped UI contracts

- A later style tag does not guarantee the final result when an earlier selector has greater specificity and both use `!important`. Before a visual patch, enumerate every matching injected phase and consolidate the owning surface; another override layer is a discarded fix.
- Testbed geometry is trustworthy only when the live root tag and parent chain match the fixture. The live article uses `article > div.view_content_wrap`; the old fixture used `article.view_content_wrap` and falsely passed its outer-shell selector.
- Numeric `z-index` is not popup coverage. `backdrop-filter`, transforms, opacity, isolation, or a positioned ancestor with its own z-index can create a trapping stacking context. Assert `elementFromPoint` on visible popup controls and remove or promote the actual trapping ancestor.
- Positive bounds matter: author width must contain the rendered writer, and title decorations must remain within a small distance of the title. Assertions that also pass at zero width do not protect usability.
- Keep the full visual map compact in `docs/ui-surface-contracts.md`; store transient measurements in `.codex/live-findings.md` and compress them after the correction. Do not build a full DOM database that will stale on host rerenders.
- `3.5.3` is a useful lifecycle/event reference, not a visual baseline. Its shared shortcut code confirms the new failure comes from later theme layer promotion, not from the modal's open/close logic.
- Geometry tests must select a rendered positive-area instance when the page intentionally keeps hidden originals or filtered mirrors. Do not weaken the positive-width assertion to accommodate a hidden selector hit.
- Wait for the owning runtime subscriber/class before dispatching an interaction in a focused page. For latency, observe the state transition directly; a throttled `requestAnimationFrame` timestamp can misreport a layer that was already positioned.
- Lifecycle tests should wait for timers/frames to return to their pre-test baseline, not add an arbitrary sleep or sample a known cleanup timer in flight. A bounded delayed-popup retry may be interaction-owned, but it must settle and remain covered by lifecycle counts.

## 2026-07-28 - Live geometry corrections after the reference review

- A grid conversion does not neutralize host positioning. Live delete pages keep `.btn_svc { position:absolute; inset:0; width:100% }`; reset position, inset, float, and transform on the password card's direct buttons before relying on two-column geometry. The fixture now carries that host collision rule and asserts non-overlap.
- Do not size an image-comment shell from the source image. Narrow article images collapsed the wrapper, box, list, and composer to 120–190px. Reuse the existing mutation-bus scheduler only to remove obsolete inline width/margin constraints; the image-comment shell follows the readable content rail.
- A small host door plus a moderate radius is still a pill. Gallery page-head doors need an actual 38–40px hit box and a 10px radius. Scope this to direct page-head children so relation/rank/guide popup descendants remain native.
- The old 1120px write cap was a host-geometry preservation decision, not a form contract. The approved wide reference uses a fluid form capped at 1400px; retain the original form, toolbar overflow, popup positioning, submit/cancel behavior, and desktop-site-mobile scaling. Padding in the scaled desktop-site mode must remain unscaled-logical 8px or it consumes about 40 physical pixels.
- Authenticated delete confirmation should remain horizontally bounded but follow the gallery head instead of vertically centering an isolated card across the viewport. Preserve method, action, hidden fields, handler, and submit type.
- Post-change validation: mobile Testbed 105/105, PC Testbed 105/105, `verify-repo all` passed. Mobile `3.5.4-beta` SHA-256 is `E48D470D45447F599FC54AB0349DE26193FAA6D42C43DF2E585387DF571D7983`; PC `1.9.9` remains `DA631A656BB919696B5936F89DF88EAD432DD95445AD99F5007CED478F4DC926`.

## 2026-07-28 - Selective glass hierarchy and native popup ownership

- Glass is a hierarchy, not a blanket effect. Keep real blur on global chrome and DCUF-owned overlays; use one painted shell plus flat divided rows for lists/comments, and an opaque reading surface for article/editor content. Individual repeated rows must not add blur, rounded cards, or elevation.
- Active palette colour is a tint inside glass, not a solid fill. Preserve selection through luminance, a thin boundary, top reflection, and shallow depth; the 14 palette IDs and stored value contract remain unchanged.
- DCInside-owned popups retain native DOM, inner layout, scrolling, colours, and event handlers. Broad article/write/list selectors must explicitly exclude popup subtrees. Correct only stacking, clipping, or viewport containment.
- `backdrop-filter` on an ancestor creates a containing block for fixed descendants in Chromium. This displaced the original article Pum popup; remove blur from reading/list ancestors rather than compensating with more offsets.
- `#hot_rank_pop2` cannot render under the host issue container after that ancestor becomes `display:none`. Portal the exact original node to `body`, mark it for idempotence, and keep the drawer clone presentation-only with popup/script/template nodes removed.
- Author-menu hover and preview hover share list rows. While a native author popup is visible, suppress row lift and preview activation so competing mouse transitions cannot flicker the popup.
- Login is an exact-host/path surface. It may read only the existing `dcuf_mobile_ui_palette` key once, never reads form values, and must preserve populated advertisements and the native form contract. Reapply the already-read palette after `DOMContentLoaded` because an attribute set at `document-start` can be replaced by the HTML parser.
- Visual Testbed roles now distinguish `shell`, `surface`, `flat`, `structural`, `input`, and `readable` materials. An approved redesign may supersede exact old radius/colour/card expectations, but storage, behavior, event, accessibility, geometry, lifecycle, and containment assertions remain mandatory.
...
## 2026-07-26 - Glass remodeling visual follow-up

- A translucent fill does not read as glass over DCInside's flat white/black canvases. The final material uses a low-contrast fixed ambient gradient behind host chrome, then distinct alpha levels for shells, nested panes, cells, inputs, and controls. `--dcuf-glass-panel-solid` is now a reduced-transparency/fallback surface rather than the default visible shell.
- Avoid broad high-alpha white gradients such as an `.82` highlight over most of a card; they turn translucent panels back into white plastic. Use a thin bright rim, a short diagonal sheen, one low-opacity accent bloom, and an outer depth shadow instead.
- GNB is a floating glass navigation rail, not an active solid-accent control. Search fields are capsule shells with the existing submit control positioned as a circular lens; preserve the host icon pseudo-elements and keep the bottom-search outer border at zero to avoid a double frame.
- Repeated list/comment cells simulate the same material without `backdrop-filter`; reserve real blur for header rails, transient popups, preview, login, and DCUF panels. This retains scrolling performance while still exposing the ambient background through alpha cells.
- On viewports at or below 520 px, the filter threshold and guest-control group must stack to one column. Retaining the old two-column rail compresses the threshold label into a vertical strip even though all controls technically remain contained.
- Do not globally force z-index on every DCUF popup. Backup/management panels participate in overlay-owned stacking and a lower forced z-index places the panel below its own touch-catching overlay, breaking pinch resize. Raise only the overlay-free settings/headtext panels that need to clear the floating menu.
- Host action popups now keep their original nodes, handlers, geometry, and placement but use light/dark glass shell, inner pane, and control tokens. Their regression contract should compare geometry across themes while expecting material colors to follow the active theme.
- Final remodeled mobile `3.5.4-beta` and shared PC `1.9.9` completed 101/101 Testbed checks each. Repository verification passed with mobile SHA-256 `2B0B0A791EE5EA0ECDFA5E3C50362A6D42A9DCB559CDD348231265E2D47F0BF1` and PC SHA-256 `1C1DC8642F5716323D1373CE701DEA57993A4FDCE284E90917C0A590CA27BB88`. Live DCInside/Tampermonkey visual smoke remains user-led.

## 2026-07-26 - Superseding obsolete visual Testbed contracts

- A deterministic UI test can preserve a regression when it asserts an implementation detail that the product has intentionally replaced. The rejected glass pass still required exact solid accent button colours and treated every nested section as an opaque bordered panel, so a green palette could contaminate the page chrome while the suite remained green.
- For an explicitly approved redesign, update the fixture and its visual assertions with the product change. Classify inspected nodes as outer shell, nested surface, input/control, or structural container: shells require material, boundary, depth, and (where intended) blur; nested surfaces require legible translucency but not another full shadow; inputs require a visible control boundary; structural containers may be intentionally transparent.
- Do not relax behavior to make appearance tests pass. Storage, filtering, event ownership, form contracts, popup geometry, viewport containment, touch/drag/preview lifecycle, contrast, and no-overflow contracts stay independent of the selected material.
- Parse modern computed colours directly, including `rgba(...)` and `color(srgb ... / alpha)`. Canvas round-tripping can resolve unsupported computed colour syntax to opaque black and falsely report alpha `1`.
- Visual evidence is incomplete when all snapshots use one palette. Include at least one light/wide non-default palette so palette-to-ambient separation and accidental full-page tinting are visible.

## 2026-07-26 - Shared glass material and login surface

- The UI material system lives in `ThemeModule`: keep the 14 palette IDs and `dcuf_mobile_ui_palette` storage contract, derive readable active-control foregrounds from each strong accent, and expose `--dcuf-glass-*` tokens for page, panel, input, inactive/active controls, boundaries, highlights, shadows, and blur. Mobile host CSS stays inside `DCUF_MOBILE_THEME_CSS_*`; shared filter/management popup material stays inside the PC-extracted shared palette rail.
- Use backdrop blur only on navigation/control bars and transient DCUF panels. List cards, article content, comments, and editor content remain comparatively opaque and use borders/highlights/shadows rather than per-card blur. Keep opaque fallbacks and reduced-motion/reduced-transparency overrides.
- Active controls retain their palette strong color as a deterministic contrast base, then add a layered highlight/softened gradient and bounded depth. Checked switches retain the existing input/events and add a neutral glass OFF track, layered accent ON track, and white shadowed knob.
- `sign.dcinside.com/login*` is mobile metadata only. `login-surface.js` executes before gallery bootstrap, installs style only on exact host/path, and reads only the existing palette key; it never reads form values or credentials. Gallery bootstrap still exits before boot hiding, observers, menus, or filter modules. Login follows OS `prefers-color-scheme`.
- Login Testbed coverage must preserve form method/action, hidden return fields, input names, save-ID/security controls, host submit listener, one allowed palette read, zero writes/menus, and absence of gallery runtime at 390×844 and 1280×900 in light/dark.
- Playwright init scripts can sample a fixture before the production bootstrap has claimed the page. Boot leak assertions should evaluate frames with an owned boot state (`locked`, `preparing`, or `degraded`), not the empty pre-bootstrap harness state; paint visibility must include ancestor body visibility/opacity.
- Mobile `3.5.4-beta` and PC `1.9.9` completed the final expanded Testbed at 101/101 each. `verify-repo all` passed with mobile SHA-256 `703CF30F51E370C1C07C984738C2199A50969803C875FC23F2D908680E00C1EB` and PC SHA-256 `FD3B64AEB2FF72500909BA473B4B59F8C5BEF9FB5B4926767371AD78B5106407`. Deterministic light/dark wide/mobile screenshots cover list, settings, preview, view, write, and login; the final beta was not installed or smoked on live DCInside.
