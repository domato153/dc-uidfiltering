...
<div class="newvisit_history"><strong class="tit">최근 방문</strong><div class="newvisit_box"><ul class="newvisit_list"><li><a class="on" href="#recent">테스트 갤러리</a></li></ul></div><button class="bnt_newvisit_more" type="button">전체</button></div></header>`;
...
export function listPage({ variant = 'major' } = {}) {
    ...
    return `${baseHead(`DCUF ${variant} list fixture`)}<body data-fixture-page="list" data-fixture-variant="${variant}">${hostChrome}${controls}${galleryHeading(variant)}${galleryDoor}<div id="top" class="dcwrap width1160 list_wrap"><main id="container" class="listwrap clear"><section class="left_content"><article><section class="gall_listwrap list">${listTable(variant)}</section><div class="bottom_paging_wrap"><div class="bottom_paging_box"><a href="${listPath}?id=test&page=2">2</a></div><div class="bottom_movebox"></div></div><div class="list_bottom_btnbox"></div><form name="frmSearch"><input name="search_keyword"><button>검색</button></form><div id="searchTypeLayer"></div></article></section></main></div><a id="fixture-to-view" href="${viewPath}?id=test&no=1001">본문으로 이동</a>${scripts}`;
}
