...
if (document.body.dataset.fixturePage === 'list') {
    ...
    if (headtext && headtextHost) {
        ...
        headtextHost.appendChild(headtext);
    }
...
const normalCommentComposer = `<div class="cmt_write_box fixture-normal-comment-composer">
    ...
    <div class="cmt_txt_cont"><div class="cmt_write"><textarea aria-label="comment body"></textarea></div><div class="cmt_cont_bottm"><div class="dccon_guidebox" style="position:relative">
        <button type="button" class="fixture-comment-dccon-trigger" onclick="const layer=document.getElementById('dccon_guide_lyr');layer.style.display=layer.style.display==='none'?'block':'none'">디시콘</button>
        <div id="dccon_guide_lyr" class="pop_wrap type2 fixture-comment-dccon-layer" style="display:none;position:absolute;right:0;bottom:42px;width:min(640px,calc(100vw - 24px));height:320px">
            <div class="pop_head">디시콘</div><div class="dccon_list_wrap"><button type="button"><img alt="테스트 댓글 디시콘" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw=="></button></div>
        </div>
    </div><div class="fr"><button type="button" class="btn_blue">Register</button></div></div></div>
</div>`;

const imageCommentComposer = `<div id="img_cmt_write_box_1" class="cmt_write_box fixture-image-comment-composer">
    <div class="fl"><div class="user_info_input"><input id="img_cmt_name_1" name="gall_nick_name" type="text" aria-label="image comment nickname"><input id="all_nick_name_1" name="all_nick_name" type="hidden"></div></div>
    <div class="cmt_txt_cont"><div class="cmt_write"><textarea aria-label="image comment body"></textarea></div><div class="cmt_cont_bottm"><div class="fr"><button type="button" class="btn_blue fixture-image-comment-submit" onclick="window.__fixtureImageCommentSubmitCount=(window.__fixtureImageCommentSubmitCount||0)+1;window.__fixtureImageCommentSubmission={nickname:document.getElementById('img_cmt_name_1').value,body:this.closest('.cmt_write_box').querySelector('textarea').value}">Register</button></div></div></div>
</div>`;
...
const pumPopupBootstrap = `<script>
window.__fixturePumOpenCount = 0;
window.Pum = {
    write_open() {
        const host = document.querySelector('.recom_bottom_box');
        if (!document.getElementById('write_pum_layer')) {
            host?.insertAdjacentHTML('beforeend', '<div id="write_pum_layer" class="pop_wrap type3" style="display:none;bottom:139px;left:50%;margin-left:-295px;width:590px;height:780px;background:#fff"><div class="pop_content ly_cloned" style="height:100%"><div class="inner"><div class="ly_cloned_cont">펌 작성 팝업</div></div></div><button type="button" class="poply_whiteclose" onclick="this.parentElement.remove()">닫기</button></div>');
        }
        const popup = document.getElementById('write_pum_layer');
        window.__fixturePumOpenCount += 1;
        popup.style.display = 'block';
        popup.style.bottom = '139px';
        popup.style.left = '50%';
        popup.style.marginLeft = '-295px';
    }
};
...
export function viewPage({ long = false, massComments = 0, variant = 'major', darkAtStart = false, brokenTheme = false } = {}) {
    ...
    const brokenThemeBootstrap = brokenTheme ? `<script>document.querySelector('#container > article > .view_content_wrap')?.style.setProperty('border-radius','0','important')</script>` : '';
    const viewHeader = `<script>document.currentScript.closest('article').classList.remove('view_content_wrap')</script><div class="view_content_wrap"><header><div class="gallview_head">
        <div class="title_headtext">[일반]</div><span class="title_subject">테스트 본문 제목</span>
        <div class="gall_writer ub-writer" data-uid="safe-view-writer" data-nick="테스트작성자">
            <div class="fl"><span class="nickname"><em>테스트작성자</em></span><span class="ip">(127.0)</span><span class="gall_date" title="2026.07.28 17:46:59">2026.07.28 17:46:59</span></div>
            <div class="fr"><span class="gall_count">조회 20</span><span class="gall_reply_num">댓글 4</span><span class="gall_recommend">추천 0</span></div>
        </div>
    </div></header>`;
    const articleBody = `<div class="gallview_contents"><section class="writing_view_box"><div class="write_div"><p>본문 시작</p><img class="fixture-narrow-article-image" data-fileno="1" alt="" src="data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==" style="display:block;width:120px;height:80px">${long ? longArticleNodes(1600) : ''}<div id="fixture-long-article"></div></div></section></div>`;
    return `${baseHead(`DCUF ${variant} view fixture`, { dark: darkAtStart })}<body class="${darkAtStart ? 'dc-filter-dark-mode' : ''}" data-fixture-page="view" data-fixture-variant="${variant}">${darkBootstrap}${hostChrome}${controls}${galleryHeading(variant)}<main id="container" class="clear ${viewClass}"><article class="view_content_wrap">${viewHeader}${articleBody}<div class="view_ad_wrap" id="fixture-initial-ad"><iframe id="google_ads_iframe_fixture" title="advertisement"></iframe></div><div class="gall_exposure_list fixture-synthetic-related"><ul><li><span class="ub-writer" data-uid="safe-related-1" data-nick="관련글작성자"></span><a href="${viewPath}?id=test&no=2001">관련 글 1</a></li><li><span class="ub-writer" data-uid="blocked-related-user" data-nick="관련차단"></span><a href="${viewPath}?id=test&no=2002">관련 글 2</a></li></ul></div>${recommendBox}${pumPopupBootstrap}${miniButtons}</article><section id="focus_cmt"><div id="comment_wrap_1" class="gall_comment comment_wrap show"><div class="comment_count"><span class="num_box"><span class="font_red">${massComments || baseCommentCount}</span></span><div class="fr"><button type="button" class="btn_cmt_refresh" data-no="1001" data-sort="">새로고침</button></div></div><div class="comment_box"><ul class="cmt_list add">${comments}</ul></div></div>${normalCommentComposer}${replyCommentComposer}</section><section class="view_comment image_comment fixture-synthetic-image-comments"><div class="comment_wrap"><div class="comment_box img_comment_box" data-imgno="1"><ul class="cmt_list">${imageCommentItem(1, { uid: 'safe-image-user' })}${imageCommentItem(2, { uid: 'blocked-image-user' })}</ul></div>${imageCommentComposer}</div></section><div class="view_bottom"><a href="${listPath}?id=test">목록</a><section class="gall_listwrap fixture-view-list">${listTable(variant, 'fixture-view-table')}</section></div></main><a id="fixture-to-list" href="${listPath}?id=test">목록으로 이동</a>${brokenThemeBootstrap}${scripts}`;
}
